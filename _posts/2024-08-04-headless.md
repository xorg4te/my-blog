...
Yes, we have python3 it's perfect :

```bash
/usr/bin/python3
```

With python, it's very simple to be able to stabilize a shell :

```bash
python3 -c 'import pty; pty.spawn("/bin/bash")'
```

We will improve our shell compatibility by editing the `TERM` environment variable :
